...
*                                  *
* K. Hinsen                        *
* Last revision: November 12, 1993 *
************************************
...
* copy coefficient arrays
      ...
      implicit none
      integer n
      real c1(0:n,0:n,0:n,4),c2(0:n,0:n,0:n,4)
...
* compress coefficients
      ...
      implicit none
      integer n0,n1,n2,n3
      real cc(n0,4,2),c(0:n1,0:n1,0:n1,4)
...
* load coefficients for s1 and s2
      ...
      common /hcoeff/ s1,s2
      real s1(0:9,0:9,0:9,4),s2(0:9,0:9,0:9,4)
...
* generate coefficients for dipole interactions (periodic bc)
      ...
      real s1c(0:9,0:9,0:9,4),s2c(0:9,0:9,0:9,4)
     ...
     .              s2zzzx,s2xxyz,s2zzxy
      real s1(84,4,2)
      real s1x(84,4,2),s1z(84,4,2)
      real s1xx(120,4,2),s1zz(120,4,2),s1xy(120,4,2),s1xz(120,4,2)
      real s2xx(84,4,2),s2zz(84,4,2),s2xy(84,4,2),s2xz(84,4,2)
      real s2xxx(84,4,2),s2zzz(84,4,2),s2xxy(84,4,2)
      real s2xxz(84,4,2),s2zzx(84,4,2),s2xyz(84,4,2)
      real s2xxxx(120,4,2),s2zzzz(120,4,2),s2xxyy(120,4,2)
      real s2xxzz(120,4,2),s2xxxy(120,4,2),s2xxxz(120,4,2)
      real s2zzzx(120,4,2),s2xxyz(120,4,2),s2zzxy(120,4,2)

      real c(0:9,0:9,0:9,4)
      integer i,j
...
* calculate interaction matrix with pbc (up to 1/r^3)
     ...
     .     s2zzzx,s2xxyz,s2zzxy
      ...
      real d0s1,d1s1(3),d2s1(6)
      real d2s2(6),d3s2(10),d4s2(15)
      real p1(120),p2(120)
      real x,y,z,xx,yy,zz,xi,yi,zi,xs,ys,zs,xi2,yi2,zi2
      real r,r2,r3,box1,box2,box3,f1,f2,f21,f22,f23
      ...
      z = abs(zc/box)
      type = 0
      if (x.ge.0.25 .neqv.  y.ge.0.25) then
         if (x.ge.0.25 .neqv.  z.ge.0.25) then
            call swap(x,z)
            type = 1
         else
            call swap(y,z)
      ...
      index = 1
      if (x .ge. 0.25) index = index+1
      if (y .ge. 0.25) index = index+1
      if (z .ge. 0.25) index = index+1
      xx = min(x,0.5-x)
      yy = min(y,0.5-y)
      zz = min(z,0.5-z)
      ...
      d2s1(1) = sum(s1xx(1,index,ti),p1,terms)-4.18879020478639098458
      d2s1(2) = xx*yy*sum(s1xy(1,index,ti),p1,terms)
      d2s1(3) = xx*zz*sum(s1xz(1,index,ti),p1,terms)
      d2s1(4) = sum(s1xx(1,index,ti),p2,terms)-4.18879020478639098458
      d2s1(5) = yy*zz*sum(s1xz(1,index,ti),p2,terms)
      d2s1(6) = sum(s1zz(1,index,ti),p1,terms)-4.18879020478639098458
 
      d4s2(1) = sum(s2xxxx(1,index,ti),p1,terms)-2.51327412287183459075
      d4s2(2) = xx*yy*sum(s2xxxy(1,index,ti),p1,terms)
      d4s2(3) = xx*zz*sum(s2xxxz(1,index,ti),p1,terms)
      d4s2(4) = sum(s2xxyy(1,index,ti),p1,terms)-0.83775804095727819691
      d4s2(5) = yy*zz*sum(s2xxyz(1,index,ti),p1,terms)
      d4s2(6) = sum(s2xxzz(1,index,ti),p1,terms)-0.83775804095727819691
      ...
      d4s2(11) = sum(s2xxxx(1,index,ti),p2,terms)-2.51327412287183459075
      d4s2(12) = yy*zz*sum(s2xxxz(1,index,ti),p2,terms)
      d4s2(13) = sum(s2xxzz(1,index,ti),p2,terms)-0.83775804095727819691
      d4s2(14) = yy*zz*sum(s2zzzx(1,index,ti),p2,terms)
      d4s2(15) = sum(s2zzzz(1,index,ti),p1,terms)-2.51327412287183459075

      if (x*x+y*y+z*z .gt. 0.) then
         do 100 i = 1,numimg(index)
            xi = x-nnimg(0,i,index)
            yi = y-nnimg(1,i,index)
            zi = z-nnimg(2,i,index)
            r2 = 1./(xi*xi+yi*yi+zi*zi)
            r = r2**0.5
            r3 = r*r*r
            xi = xi*r
            yi = yi*r
            ...
            d2s1(1) = d2s1(1) + r3*(3.*xi2-1.)
            d2s1(2) = d2s1(2) + 3.*r3*xi*yi
            d2s1(3) = d2s1(3) + 3.*r3*xi*zi
            d2s1(4) = d2s1(4) + r3*(3.*yi2-1.)
            d2s1(5) = d2s1(5) + 3.*r3*yi*zi
            d2s1(6) = d2s1(6) + r3*(3.*zi2-1.)
 
            d2s2(1) = d2s2(1) + 0.5*r*(1.-xi2)
            d2s2(2) = d2s2(2) - 0.5*r*xi*yi
            d2s2(3) = d2s2(3) - 0.5*r*xi*zi
            d2s2(4) = d2s2(4) + 0.5*r*(1.-yi2)
            d2s2(5) = d2s2(5) - 0.5*r*yi*zi
            d2s2(6) = d2s2(6) + 0.5*r*(1.-zi2)
 
            d3s2(1) = d3s2(1) - 1.5*r2*xi*(1.-xi2)
            d3s2(2) = d3s2(2) - 0.5*r2*yi*(1.-3.*xi2)
            d3s2(3) = d3s2(3) - 0.5*r2*zi*(1.-3.*xi2)
            d3s2(4) = d3s2(4) - 0.5*r2*xi*(1.-3.*yi2)
            d3s2(5) = d3s2(5) + 1.5*r2*xi*yi*zi
            d3s2(6) = d3s2(6) - 0.5*r2*xi*(1.-3.*zi2)
            d3s2(7) = d3s2(7) - 1.5*r2*yi*(1.-yi2)
            d3s2(8) = d3s2(8) - 0.5*r2*zi*(1.-3.*yi2)
            d3s2(9) = d3s2(9) - 0.5*r2*yi*(1.-3.*zi2)
            d3s2(10) = d3s2(10) - 1.5*r2*zi*(1.-zi2)
 
            d4s2(1) = d4s2(1) + 1.5*r3*(xi2*(6.-5.*xi2)-1.)
            d4s2(2) = d4s2(2) + 1.5*r3*xi*yi*(3.-5.*xi2)
            d4s2(3) = d4s2(3) + 1.5*r3*xi*zi*(3.-5.*xi2)
            d4s2(4) = d4s2(4) + 0.5*r3*(-15.*xi2*yi2+3.*(xi2+yi2)-1.)
            d4s2(5) = d4s2(5) + 1.5*r3*yi*zi*(1.-5.*xi2)
            d4s2(6) = d4s2(6) + 0.5*r3*(-15.*xi2*zi2+3.*(xi2+zi2)-1.)
            d4s2(7) = d4s2(7) + 1.5*r3*xi*yi*(3.-5.*yi2)
            d4s2(8) = d4s2(8) + 1.5*r3*xi*zi*(1.-5.*yi2)
            d4s2(9) = d4s2(9) + 1.5*r3*xi*yi*(1.-5.*zi2)
            d4s2(10) = d4s2(10) + 1.5*r3*xi*zi*(3.-5.*zi2)
            d4s2(11) = d4s2(11) + 1.5*r3*(yi2*(6.-5.*yi2)-1.)
            d4s2(12) = d4s2(12) + 1.5*r3*yi*zi*(3.-5.*yi2)
            d4s2(13) = d4s2(13) + 0.5*r3*(-15.*yi2*zi2+3.*(yi2+zi2)-1.)
            d4s2(14) = d4s2(14) + 1.5*r3*yi*zi*(3.-5.*zi2)
            d4s2(15) = d4s2(15) + 1.5*r3*(zi2*(6.-5.*zi2)-1.)
100      continue
      endif
      ...
      f1 = 8.37758040957278196917*box3
      f2 = 2.51327412287183459075*box3
      f21 = 0.5*f2
      f22 = -f2/3.
      f23 = -2.*f22
      ...
      g0s1s(1,1) = 0.5*d1s1(2)-d3s2(9)
      g0s1s(1,2) = -d3s2(8)
      g0s1s(1,3) = 0.5*d1s1(1)-d3s2(6)
      g0s1s(1,4) = -d3s2(5)
      g0s1s(1,5) = -d3s2(3)
      g0s1s(2,1) = 0.5*d1s1(3)-d3s2(8)
      g0s1s(2,2) = d1s1(2)-d3s2(7)
      g0s1s(2,3) = -d3s2(5)
      g0s1s(2,4) = 0.5*d1s1(1)-d3s2(4)
      g0s1s(2,5) = -d3s2(2)
      g0s1s(3,1) = -d3s2(5)
      g0s1s(3,2) = -d3s2(4)
      g0s1s(3,3) = 0.5*d1s1(3)-d3s2(3)
      g0s1s(3,4) = 0.5*d1s1(2)-d3s2(2)
      g0s1s(3,5) = d1s1(1)-d3s2(1)
 
      g0s2s(1,1) = d2s1(4)/3.+d2s1(6)/15.-d4s2(13)
      g0s2s(1,2) = 0.2*d2s1(5)-d4s2(12)
      g0s2s(1,3) = d2s1(2)/3.-d4s2(9)
      g0s2s(1,4) = d2s1(3)/15.-d4s2(8)
      g0s2s(1,5) = d2s1(1)/3.+d2s1(6)/15.-d4s2(6)
      g0s2s(1,6) = d2s1(5)/15.-d4s2(5)
      g0s2s(1,7) = 0.2*d2s1(3)-d4s2(3)
      g0s2s(2,1) = 11.*d2s1(5)/15.-d4s2(12)
      g0s2s(2,2) = 1.2*d2s1(4)-d4s2(11)
      g0s2s(2,3) = d2s1(3)/3.-d4s2(8)
      g0s2s(2,4) = 11.*d2s1(2)/15.-d4s2(7)
      g0s2s(2,5) = d2s1(5)/15.-d4s2(5)
      g0s2s(2,6) = d2s1(1)/3.+d2s1(4)/15.-d4s2(4)
      g0s2s(2,7) = 0.2*d2s1(2)-d4s2(2)
      g0s2s(3,1) = d2s1(3)/15.-d4s2(8)
      g0s2s(3,2) = 0.2*d2s1(2)-d4s2(7)
      g0s2s(3,3) = d2s1(5)/3.-d4s2(5)
      g0s2s(3,4) = d2s1(4)/3.+d2s1(1)/15.-d4s2(4)
      g0s2s(3,5) = 11.*d2s1(3)/15.-d4s2(3)
      g0s2s(3,6) = 11.*d2s1(2)/15.-d4s2(2)
      g0s2s(3,7) = 1.2*d2s1(1)-d4s2(1)
 
      g1s1s(1,1) = 0.25*(d2s1(4)+d2s1(6))-d4s2(13)+f21
      g1s1s(1,2) = 0.5*d2s1(5)-d4s2(12)
      g1s1s(1,3) = 0.25*d2s1(2)-d4s2(9)
      g1s1s(1,4) = 0.25*d2s1(3)-d4s2(8)
      g1s1s(1,5) = -d4s2(5)
      g1s1s(2,1) = g1s1s(1,2)
      g1s1s(2,2) = d2s1(4)-d4s2(11)+f23
      g1s1s(2,3) = -d4s2(8)
      g1s1s(2,4) = 0.5*d2s1(2)-d4s2(7)
      g1s1s(2,5) = -d4s2(4)+f22
      g1s1s(3,1) = g1s1s(1,3)
      g1s1s(3,2) = g1s1s(2,3)
      g1s1s(3,3) = 0.25*(d2s1(1)+d2s1(6))-d4s2(6)+f21
      g1s1s(3,4) = 0.25*d2s1(5)-d4s2(5)
      g1s1s(3,5) = 0.5*d2s1(3)-d4s2(3)
      g1s1s(4,1) = g1s1s(1,4)
      g1s1s(4,2) = g1s1s(2,4)
      g1s1s(4,3) = g1s1s(3,4)
      g1s1s(4,4) = 0.25*(d2s1(1)+d2s1(4))-d4s2(4)+f21
      g1s1s(4,5) = 0.5*d2s1(2)-d4s2(2)
      g1s1s(5,1) = g1s1s(1,5)
      g1s1s(5,2) = g1s1s(2,5)
      g1s1s(5,3) = g1s1s(3,5)
      g1s1s(5,4) = g1s1s(4,5)
      ...
      g0s1t(1,1) = 0.5*d2s1(3)
      g0s1t(1,2) = d2s1(2)
      g0s1t(1,3) = -0.5*d2s1(5)
      g0s1t(1,4) = 0.5*(d2s1(1)-d2s1(4))
      g0s1t(1,5) = -d2s1(2)
      g0s1t(2,1) = -0.5*d2s1(2)
      g0s1t(2,2) = 0.
      g0s1t(2,3) = 0.5*(d2s1(6)-d2s1(1))
      g0s1t(2,4) = 0.5*d2s1(5)
      g0s1t(2,5) = d2s1(3)
      g0s1t(3,1) = 0.5*(d2s1(4)-d2s1(6))
      g0s1t(3,2) = -d2s1(5)
      g0s1t(3,3) = 0.5*d2s1(2)
      g0s1t(3,4) = -0.5*d2s1(3)
      g0s1t(3,5) = 0.
