...
120   continue
      ...
      integer i,j,k
#ifdef IMSL
      double precision eval(nmax),evec(nmax,nmax),ievec(nmax,nmax)
#endif
#ifdef LAPACK
      integer lwork
...
120   continue
      ...
      double precision trm(im,nc,nc),trms(im,nc,nc)
...
500   continue
      ...
      do 600 l = 0,lm
         if (l .gt. 0) fl = l*fl
         z(l,1) = -(2.d0*l+3.d0)*(1.d0-xi)/
     .        (fl*(1.d0+2.d0*l*xi)*(l+2.d0)*dfact(2*l-1))
         z(l,2) = -(1.d0-(l+3.d0)*xi)/(fl*(1.d0+l*xi)*(l+2.d0)
     .        *dfact(2*l+1))
         z(l,3) = -(2.d0*l+3.d0)**2*(1.d0-5.d0*xi)/
     .        (4.d0*fl*(1.d0+2.d0*l*xi)*(l+2.d0)*dfact(2*l+5))
         z(l,4) = -(1.d0-3.d0*xi)/(2.d0*fl*(l+2.d0)*(1.d0+2.d0*l*xi)
     .        *dfact(2*l-1))
600   continue
...
* Calculate powers of r and x/r
      ...
      double precision px(3,0:2*lm+2),pr(0:2*lm+3)
      common /po/ px,pr

      integer i
      double precision r,xh,yh,zh
...
* Evaluate terms for one component
      ...
      double precision function ev(si,ei)
      implicit none
      integer si,ei
...
* Insert a contribution to G into the final matrix
      ...
      logical tr
      double precision g(dim,dim),gx(ngx,ngx),f
...
#ifdef PERIODIC
      parameter (cutoff = 6.d0)
#else
      parameter (cutoff = 1.d6)
#endif
...
#endif
      logical ho,first
      double precision xl,yl,zl,s,st,r
      double precision gx(4*lm+5,4*lm+5)
      double precision temp(dim,2*lm+3)
...
#ifdef PERIODIC
      call gpbc(xc,yc,zc)
      ns = idint((2*cutoff+box-0.1d0)/box)-1
      do 150 nx = -ns,ns
      xl = xc + nx*box
      do 151 ny = -ns,ns
      yl = yc + ny*box
...
#endif
      r = xl*xl+yl*yl+zl*zl
      ho = (r .le. cutoff*cutoff) .and. (r .gt. 0.d0)
      if (ho) call powers(xl,yl,zl)

      do 200 m = 0,lm
         st = 1.d0-2.d0*mod(m,2)
         do 210 k = m,lm
            s = 1.d0-2.d0*mod(k,2)
